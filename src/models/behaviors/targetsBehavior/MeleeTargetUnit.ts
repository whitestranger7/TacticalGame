    ...
    getTargets(Board: BoardModel) {
        ...
        if(this.id >= start && this.id <= end){
            ...
            if(this.id <= (start + cols - 1)){
                let targets: number[] = [];
                if(this.id === start) {
                    checkTargetAlive(Board, targets, [this.id+cols, this.id+cols+1]);
                    if(targets.length === 0) {
                        checkTargetAlive(Board, targets, [this.id+cols+2]);
                    }
                    if(targets.length === 0) {
                        ...
                        checkTargetAlive(Board, targets, newTargets);
                    }
                    return targets;
                }
                if(this.id === (start + cols - 1)) {
                    checkTargetAlive(Board, targets, [this.id+cols-1, this.id+cols]);
                    if(targets.length === 0) {
                        checkTargetAlive(Board, targets, [this.id+cols-2]);
                    }
                    if(targets.length === 0) {
                ...
                checkTargetAlive(Board, targets, [this.id+cols-1, this.id+cols, this.id+cols+1])
                if(targets.length === 0) {
                    checkTargetAlive(Board, targets, [this.id+(cols*2)-1, this.id+(cols*2), this.id+(cols*2)+1])
                }
                return targets;
